...
NewUserDialog::NewUserDialog(ThunderballApp* theApp, bool param_2, bool param_3, bool param_4)
	: ThunderDialog(10, true, param_2 ? "New User" : "Rename User", "Please enter your name.", "", 1)
{
	mApp = theApp;
	mUnk0x188 = param_3;
	...
	if (mUnk0x188) {
		...
		mUnk0x18C = StrFormat(
			ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\NewUserDialog.cpp655,37", "%d / %d / %d"),
			aMonth,
			aDay,
			aYear
		);
		...
		mUnk0x180->SetFont(
			GetFontById(ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\NewUserDialog.cpp656,39", 0x1e))
		);
		if (!param_4) {
		...
		Color aColor4 =
			Color(ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\NewUserDialog.cpp659,50", 0xffffff));
...
void NewUserDialog::Draw(Graphics* g)
{
	if (!mUnk0x188) {
		...
		g->SetFont(
			GetFontById(ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\NewUserDialog.cpp676,151", 0x1d))
		);
		...
		if (mVisible) {
			g->SetColor(
				Color(ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\NewUserDialog.cpp679,157", 0xd6cfc0))
			);
	...
	if (mUnk0x1A8 != 0) {
		...
		if (mUnk0x1A8 == ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\NewUserDialog.cpp672,120", 0x4b)) {
			...
			if (mApp->mLoadingScreen != NULL) {
				mApp->mLoadingScreen->mAdventureStartDelay =
					ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\NewUserDialog.cpp673,129", 0x14);
			}
...
// FUNCTION: POPCAPGAME1 0x0049f890
std::string NewUserDialog::GetName()
{
	return mUnk0x180->mString;	
}

// FUNCTION: POPCAPGAME1 0x00499ef0
void NewUserDialog::SetName(const std::string& theName)
{
	mUnk0x180->SetText(theName);
	mUnk0x180->mCursorPos = theName.length();
	mUnk0x180->mHilitePos = 0;
}

// FUNCTION: POPCAPGAME1 0x00495660
void NewUserDialog::DoApproved()
{
	if (mUnk0x184 != NULL) {
		mUnk0x184->SetDisabled(true);
	}
	if (mNoButton != NULL) {
