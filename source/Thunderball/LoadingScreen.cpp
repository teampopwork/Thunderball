...
ASSERT_OFFSET(LoadingScreen, mDoneLoadingCnt, 0x9C);
ASSERT_OFFSET(LoadingScreen, mPlayNowPulseCount, 0xA0);
ASSERT_OFFSET(LoadingScreen, mVisualLoadProgress, 0xA4);
...
ASSERT_OFFSET(LoadingScreen, mBallBounced, 0xC4);
ASSERT_OFFSET(LoadingScreen, mParticles, 0xC8);
ASSERT_OFFSET(LoadingScreen, mShowDeluxe, 0xD8);
ASSERT_OFFSET(LoadingScreen, mLevelLoadMusicThread, 0xDC);
ASSERT_OFFSET(LoadingScreen, mMusicThreadId, 0xE0);
ASSERT_OFFSET(LoadingScreen, mLogoAnimFrames, 0xE4);
...
LoadingScreen::~LoadingScreen()
{
	...
	if (mLevelLoadMusicThread != 0) {
		PostThreadMessageA((DWORD) mMusicThreadId, WM_QUIT, 0, 0);
...
void LoadingScreen::DrawOverlay(Graphics* g)
{
	...
	if ((0 < mPlayNowPulseCount) && (mPlayNowPulseCount < 0x32)) {
		...
		float finalAlpha =
			...
			pulseFactor * ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp429,490", 100);
	...
	if (updateCnt < fadeLimit) {
		...
		g->DrawImage(
			IMAGE_LOAD_POPCAP,
			(mWidth - IMAGE_LOAD_POPCAP->mWidth) / 2,
			(mHeight - IMAGE_LOAD_POPCAP->mHeight) / 2
		);
		...
		if (updateCnt > fadeLimit) {
			...
			int partnerFadeMax =
				...
				ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp434,523", 450);
			if (currentUpdate < partnerFadeMax) {
				if (mDisplayPartnerLogo) {
					...
					g->DrawImage(
						IMAGE_PARTNER,
						(mWidth - IMAGE_PARTNER->mWidth) / 2,
						(mHeight - IMAGE_PARTNER->mHeight) / 2
					);
					g->SetColorizeImages(false);
				}
			}
...
void LoadingScreen::Draw(Graphics* g)
{
	...
	if (g->mIs3D && mSunRotationAngle != 0.0f) {
		...
		g->DrawImageRotated(
			...
			ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp437,553", 383),
			NULL
		);
	...
	int logoX = ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp440,572", 0xaa);

	// STRING: POPCAPGAME1 0x00607740
	int finalLogoY = ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp441,573", 0x8c);

	// STRING: POPCAPGAME1 0x006076f8
	int initialLogoY = ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp442,574", 200);

	// STRING: POPCAPGAME1 0x006076b0
	int logoStartY = ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp443,575", -0x3c);

	// STRING: POPCAPGAME1 0x00607668
	int hillOffsetY = ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp444,577", 0x32);

	// STRING: POPCAPGAME1 0x00607620
	int logoRiseFrames = ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp445,578", 0x46);
	...
	if (needCalc) {
		y = initialLogoY + finalLogoY;
		needCalc = false;

		SolveLinearSystem(
			hillOffsetY * hillOffsetY,
			hillOffsetY,
			(logoStartY + finalLogoY - y),
	...
	g->SetColor(Color(
		...
		ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp450,612", 120) * mVisualLoadProgress *
			mVisualLoadProgress
	));
	...
	g->DrawImage(
		...
		hillOffsetY + ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp452,615", 290)
	);
	...
	if (mLogoAnimFrames >= hillOffsetY && mLogoAnimFrames < logoRiseFrames) {
		...
		if (mLogoAnimFrames >= logoRiseFrames) {
			g->DrawImage(IMAGE_LOAD_LOGO, logoX, finalLogoY);
			if (mLogoBounceFrame != 0) {
				g->SetColorizeImages(true);
				g->SetDrawMode(Graphics::DRAWMODE_ADDITIVE);

				// STRING: POPCAPGAME1 0x006072c0
				int alpha = ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp457,634", 128) *
							InterpValF(
								mLogoBounceFrame,
	...
	if (loadingText.length() != 0) {
		...
		g->DrawString(
			loadingText,
			loadingTextXCentered +
				// STRING: POPCAPGAME1 0x00607120
				ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp461,659", 0),
...
void LoadingScreen::Update()
{
	...
	float displayProgress =
		...
		(updateCnt - ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp398,218", 250)) *
	...
	if (mDisplayPartnerLogo) {
		...
		displayProgress =
			(float) (updateCnt -
					 ...
					 ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp400,220", 550)) *
	...
	if (mVisualLoadProgress < mLoadProgress) {
		mVisualLoadProgress = mLoadProgress;
		MarkDirty();
	}

	if (mVisualLoadProgress >= 1.0f) {
		if (mUpdateCnt > 550) {
			if (mBallAnimState == 0) {
				mVisualLoadProgress = 1.0f;
				DoBallBounce();
			}
		}
	}
...
void LoadingScreen::DrawLight(Graphics* g, int param_2)
{
	...
	int xOffset =
		(int) (local_c +
			   ...
			   (double) ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp426,472", 0xe0));
	...
	for (size_t i = 0; i < mParticles.size(); ++i) {
		...
		if ((rand() % 0xFF) < mParticles[i].mAlpha) {
			...
			int sparkY =
				...
				sparkYf - ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp423,383", 4);
			int sparkX =
				// STRING: POPCAPGAME1 0x00005fe678
				sparkXf - ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp422,383", 4);
...
void LoadingScreen::UpdateParticles()
{
	for (size_t i = 0; i < mParticles.size(); i++) {
		...
		if (mParticles[i].mAlpha <= 0) {
			...
			float speedFactor =
				0.25 - (mParticles[i].mVelX * mParticles[i].mVelY) + (mParticles[i].mVelX * mParticles[i].mVelY);
			mParticles[i].mY +=
				+mParticles[i].mVelY +
				(speedFactor *
				 // STRING: POPCAPGAME1 0x00601fa8
				 ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp415,345", 3.0f) *
				 sin(mParticles[i].mAngle));
			mParticles[i].mX +=
				mParticles[i].mVelX +
				(speedFactor *
				 // STRING: POPCAPGAME1 0x00601f60
				 ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp416,346", 3.0f) *
				 cos(mParticles[i].mAngle));

			float angleDelta;
			if (mParticles[i].mFlip) {
				angleDelta =
					// STRING: POPCAPGAME1 0x00601f10
					ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp417,350", 0.015f);
			}
			else {
				angleDelta =
					// STRING: POPCAPGAME1 0x00601ec0
					ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp418,350", -0.015f);
			}
		...
		do {
			...
			double aRandX =
				(double) (rand() %
						  ...
						  ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp406,312", 30));
			double aCosPerp = cos((double) aParticle.mAngle + (SEXY_PI / 2.0f));

			double aDistScaleX =
				// STRING: POPCAPGAME1 0x0060cd40
				Sexy::ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp405,312", 4.3f);

			double xScaled = aDistScaleX * aCosPerp * (double) i;

			aParticle.mX = (xScaled + (double) mBallX + aRandX) -
						   // STRING: POPCAPGAME1 0x0060ccf8
						   ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp407,312", 15);

			double aRandY =
				(double) (rand() %
						  // STRING: POPCAPGAME1 0x0060ccb0
						  ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp409,313", 30));
			double aSinPerp = sin((double) aParticle.mAngle + (double) (SEXY_PI / 2.0f));

			double aDistScaleY =
				// STRING: POPCAPGAME1 0x0060cc68
				Sexy::ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp408,313", 4.3f);

			double yScaled = aDistScaleY * aSinPerp * (double) i;

			aParticle.mY = (yScaled + (double) mBallY + aRandY) -
						   // STRING: POPCAPGAME1 0x0060cc20
						   ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp410,313", 15);

			double aVelScaleX =
				// STRING: POPCAPGAME1 0x0060cbd8
				Sexy::ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp411,315", -0.071f);
			aParticle.mVelX = aVelScaleX * aCosPerp * (double) i;

			double aVelScaleY =
				// STRING: POPCAPGAME1 0x0060cb90
				Sexy::ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp412,316", -0.071f);
			aParticle.mVelY = aVelScaleY * aSinPerp * (double) i;

			aParticle.mAlpha =
				// STRING: POPCAPGAME1 0x0060cb48
				Sexy::ModVal(0, "SEXY_SEXYMODVALc:\\gamesrc\\cpp\\thunderball\\LoadingScreen.cpp413,318", 255);
			aParticle.mFlip = (0 < i);
...
void LoadingScreen::LevelLoadMusicThreadProc(LPVOID lpParameter)
{
	...
	if (hFind != INVALID_HANDLE_VALUE) {
		...
		do {
			MSG msg;
			if (!PeekMessageA(&msg, NULL, WM_NULL, WM_NULL, PM_REMOVE) || msg.message == 0x12 ||
